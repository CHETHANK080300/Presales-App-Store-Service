...
public class CorporatePostHook implements PostHookProcessor {
    @Override
    public <T> T process(T response, String interfaceName) {
        // Manipulate ACC_STATUS in accounts array if present
        if (interfaceName != null) {
            if (interfaceName.toLowerCase().contains("customer360")) {
            ...
            } else if (interfaceName.toLowerCase().contains("fundtransfernapas") || interfaceName.toLowerCase().contains("fundtransfercitad")) {
                // FundTransfer-specific post-hook logic
            }
        }
        return response;
    }
